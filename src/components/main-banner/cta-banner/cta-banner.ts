...
export class CtaBanner {
  ...
  constructor() {
    ...
    const descriptionBanner = createHTMLElement("span", [
      "span-description-banner",
    ]);
    ...
    const infoNumbersBlock = createHTMLElement("div", [
      "div-info-numbers-block",
    ]);
    this.node.append(infoNumbersBlock);

    const divider = createHTMLElement("div", ["div-info-divider"]);
    const divider2 = createHTMLElement("div", [
      "div-info-divider",
      "div-info-divider-2",
    ]);
    ...
    BANNERINFO.infoNumbers.forEach((infoNumber: InfoNumbers, index: number) => {
      ...
      const numDescription = createHTMLElement("span", [
