...
export class CategoryProductCard {
  ...
  constructor(productData: Product) {
    ...
    const originalPriceElement = createHTMLElement("span", [
      "span-product-price",
      "span-original-price",
    ]);
    originalPriceElement.textContent = `$${this.price.toFixed(2)}`;

    const discountElement = createHTMLElement("span", [
      "span-product-discount",
    ]);
    const visiblePercentage =
      this.discountPercentage > 1
        ? this.discountPercentage.toFixed(0)
        : this.discountPercentage.toFixed(2);
    discountElement.textContent = `-${visiblePercentage}%`;

    const discountedPrice = this.price * (1 - this.discountPercentage / 100);
    const discountedPriceElement = createHTMLElement("span", [
      "span-product-price",
